      ...
      <div ref={referencia} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-16">
          ...
          <motion.div
            ...
            className="relative bg-dark-800/60 rounded-2xl border border-dark-600 p-8 overflow-hidden group hover:border-cyber-primary/40 transition-colors"
          >
            ...
            <div className="flex items-center space-x-3 mb-5">
              <div className="w-11 h-11 flex items-center justify-center rounded-xl bg-cyber-primary/10 border border-cyber-primary/30 flex-shrink-0">
            ...
            <p className="text-gray-300 leading-relaxed">
              {t('about.mission.text').split(t('about.mission.highlight')).map((parte, i, arr) =>
                i < arr.length - 1 ? (
                  <span key={i}>
                    {parte}
                    <span className="text-cyber-primary font-semibold">{t('about.mission.highlight')}</span>
                  </span>
                ) : parte
              )}
            </p>
          ...
          <motion.div
            ...
            className="relative bg-dark-800/60 rounded-2xl border border-dark-600 p-8 overflow-hidden group hover:border-cyber-secondary/40 transition-colors"
          >
            ...
            <div className="flex items-center space-x-3 mb-5">
              <div className="w-11 h-11 flex items-center justify-center rounded-xl bg-cyber-secondary/10 border border-cyber-secondary/30 flex-shrink-0">
                <Eye size={22} className="text-cyber-secondary" />
              </div>
            ...
            <p className="text-gray-300 leading-relaxed">
              {t('about.vision.text').split(t('about.vision.highlight')).map((parte, i, arr) =>
                i < arr.length - 1 ? (
                  <span key={i}>
                    {parte}
                    <span className="text-cyber-secondary font-semibold">{t('about.vision.highlight')}</span>
                  </span>
                ) : parte
              )}
            </p>
        ...
        >
          <h3 className="text-2xl sm:text-3xl font-bold text-white mb-2">
            {t('about.valuesTitle')}{' '}
            <span className="gradient-text">{t('about.valuesAcronym')}</span>
          </h3>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {valoresEpico.map((valor, indice) => (
            <motion.div
              ...
              animate={estaVisivel ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.6, delay: 0.5 + indice * 0.1 }}
              className="relative bg-dark-800/60 rounded-xl border border-dark-600 p-6 group hover:border-cyber-primary/40 hover:-translate-y-1 transition-all"
            >
              {/* Letra */}
