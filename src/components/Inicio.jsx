  ...
  return (
    <section id="inicio" className="relative min-h-screen flex items-center justify-center cyber-grid overflow-hidden">
      <div className="absolute inset-0 overflow-hidden" aria-hidden="true">
        <div className="absolute top-1/4 -left-32 w-96 h-96 bg-cyber-primary/10 rounded-full blur-[80px]" />
        <div className="absolute bottom-1/4 -right-32 w-96 h-96 bg-cyber-secondary/10 rounded-full blur-[80px]" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-cyber-accent/5 rounded-full blur-[100px]" />
      </div>
      <div className="absolute top-32 left-[15%] text-cyber-primary/30" aria-hidden="true"><Zap size={40} /></div>
      <div className="absolute top-48 right-[20%] text-cyber-accent/30" aria-hidden="true"><TrendingUp size={48} /></div>
      <div className="absolute bottom-40 left-[20%] text-cyber-secondary/30" aria-hidden="true"><BarChart3 size={36} /></div>
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-20 lg:pt-24">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.8 }} className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <motion.a href={linkWhatsapp} target="_blank" rel="noopener noreferrer" className="group relative inline-flex items-center space-x-3 bg-cyber-primary px-8 py-4 rounded-full text-lg font-semibold text-white overflow-hidden" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
