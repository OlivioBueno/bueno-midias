      ...
      <div ref={referencia} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={estaVisivel ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-center mb-16">
          <motion.div initial={{ opacity: 0, scale: 0.9 }} animate={estaVisivel ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.1 }} className="inline-flex items-center space-x-2 bg-cyber-primary/10 border border-cyber-primary/30 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-sm text-cyber-primary font-medium">{t('clients.badge')}</span>
          </motion.div>
          <motion.h2 initial={{ opacity: 0, y: 20 }} animate={estaVisivel ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.2 }} className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4">
            <span className="text-white">{t('clients.title')} {t('clients.titleGradient')}</span>
          </motion.h2>
            ...
            {clientes.map((cliente) => (
              <SwiperSlide key={cliente.nome}>
                <a href={cliente.url} target="_blank" rel="noopener noreferrer" className="group block h-full">
                  <div className="relative h-full bg-dark-700/60 backdrop-blur-sm rounded-2xl p-6 border border-dark-600 hover:border-cyber-primary/50 transition-all duration-500 overflow-hidden">
                    <div className="absolute top-0 right-0 w-40 h-40 bg-cyber-primary/5 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
